...
public class CopyFromNextAction extends PhotoManagerAction {
    ...
    public void actionPerformed(final ActionEvent e) {
        ...
        if (selection.getMaxSelectionIndex()==this.a_table.getRowCount()-1) return;

        int selectedColumn = this.a_table.getSelectedColumn();
        if ( selectedColumn == -1) return;

        final Object value = this.a_table.getValueAt(selection.getMaxSelectionIndex()+1,selectedColumn);
