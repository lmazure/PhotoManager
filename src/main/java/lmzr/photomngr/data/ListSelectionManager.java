...
public class ListSelectionManager {
    ...
    public int[] getSelection() {
        ...
        if ( a_selection.getMinSelectionIndex() == -1 ) {
            return new int[0];
        }
        ...
        for (int i=a_selection.getMinSelectionIndex(); i<=a_selection.getMaxSelectionIndex(); i++) {
            if (a_selection.isSelectedIndex(i)) {
    ...
    public void setSelection(final int selection[]) {
        ...
        for (int i=1; i<selection.length;i++ ) {
            a_selection.addSelectionInterval(selection[i],selection[i]);
        }
    }
    ...
    public void next(final int incr) {
        ...
        if (i>=a_list.getRowCount()) {
            i-= a_list.getRowCount();
        }
